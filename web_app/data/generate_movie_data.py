...
def get_film_names(url, year):
    ...
    for th in table_headers:
        ...
        for tr in table.find_all('tr'):
            for ind, td in enumerate(tr.find_all('td', rowspan=lambda x: x is None)):
                if ind == 0:
...
def get_rating(film_id):
    ...
    digit = int(h[m.start()])
    return random.randrange(1, 3) if digit == 3 else random.randrange(3, 6)
...
with open('ratings.dat', 'w') as file_handler:
    for user_id in range(1, num_users):
        ...
        for movie_id in movie_ids:
            ...
            if rating > 0:
                file_handler.write(f"{user_id}::{movie_id}::{rating}::N/A\n")
